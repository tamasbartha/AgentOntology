...
*A constructive ontology and constraint-grounded dynamics of agency*
...
This work presents two coupled contributions: a constructive ontology of agency and a constraint-grounded dynamics of it. The ontology compresses the empirical category of agents to a single constitutive axiom, the **constructive agent**: the finite region enclosed by a minimum-bandwidth, minimum-area cut of a feedback loop in physical space. Loops are primary; agents are what their narrowest cuts bound. The dynamics derives, from this ontology plus five physical constraints (Landauer's energy-information coupling, finite power, spatial unboundedness, the Bekenstein bound, and finite processing speed via Bremermann and Margolus-Levitin), the directly observable properties of empirical agents across eight classes from RNA viroid to human. The methodological move is the same one thermodynamics made — compress observed regularities to a small foundation, then derive their consequences.

The framework inverts the Free Energy Principle. Where FEP holds that agents act to minimize the surprise they receive from their environment, this framework derives — as a consequence of loop-priority — that agents act to maintain or increase the surprise they exert *on* their environment, because outward informativeness is what keeps the loop circulating, and loop circulation is what the agent's existence consists of. Reduced received surprise is a side effect, not the optimization target. The Dark Room paradox resolves accordingly: an agent that surprises its environment with nothing has by hypothesis ceased contributing information to the loop, and the loop short-circuits around it.

From the foundation, fifteen subchapters of derivation produce the framework's content: bandwidth finiteness and termination necessity, world model necessity, inference as similarity-based trajectory reuse, learning as logging, the substrate trade-off forcing a figure-8 architectural split, memory hierarchies, habituation and orienting, layered self-modeling, the optimization gap, the rising-and-falling shape of agent property exhibition over a lifespan, and the internal-grounding bound on individual-agent claims.

The compression succeeds at its scope: all directly observable empirical-layer properties are accounted for — most by derivation, the rest as forced consequences under stated assumptions — with substrate-specific particulars deliberately below the framework's level. The framework produces six novel observable predictions and ten falsifications of existing literature claims — including FEP surprise minimization, gradient-based learning as primary cognitive mechanism, and strict goal optimization at the individual agent level. Three major extensions are flagged for future work: multi-agent dynamics (extending the ontology to nested and shared loops), evolutionary dynamics (operating upstream of loop ignition to refine which materials can support loops), and developmental dynamics (operating during the early lifecycle as the agent's substrate is constructed). The mutual falsifiability is what makes both sides scientific.
...
Two coupled contributions: a constructive ontology of agency (what makes a region of physical space the locus of agency, with agents emerging as what loop-bottlenecks bound) and a constraint-grounded dynamics of it (how agency, once underway, evolves over the lifecycle from formation to termination). Both compress to a small foundation: five physical constraints (Landauer, finite power, spatial unboundedness, Bekenstein, Bremermann/Margolus-Levitin) plus the constructive agent — the finite region enclosed by a minimum-bandwidth, minimum-area cut of a feedback loop in physical space. Loops are primary; agents are what their narrowest cuts bound. The methodological move is the same one thermodynamics made.

The framework inverts the Free Energy Principle: agents optimize the surprise exerted *on* their environment, not received from it, because outward informativeness is what keeps the loop circulating. Reduced received surprise is a side effect; the Dark Room paradox resolves as a direct consequence. The compression succeeds at its scope — all directly observable empirical-layer properties are accounted for, most by derivation and the rest as forced consequences under stated assumptions, with substrate-specific particulars deliberately below the framework's level. The framework produces six novel observable predictions and ten falsifications of existing literature claims. Three major extensions are flagged for future work: multi-agent dynamics (extending the ontology to nested and shared loops), evolutionary dynamics (refining what materials can support loops, upstream of loop ignition), and developmental dynamics (constructing operational substrate during the early lifecycle). The mutual falsifiability is what makes both sides scientific.
...
- **Chapter 2: The Statement**
  - 2.1 Loops, Bottlenecks, and the Constructive Agent
  - 2.2 The Predictive Claim
  - 2.3 The Deductive Bridge
  - 2.4 The Central Claim
...
Beyond the empirical category established in Chapter 1, we now propose a constructive definition that picks out agents by their topological structure. The two definitions — empirical and constructive — are meant to coincide: the document's central thesis is that any entity matching the constructive definition exhibits the empirical properties, and vice versa.

The constructive definition takes feedback loops, not agents, as the primary object. A **feedback loop**<sup>16</sup> is a self-closing channel in which information flows around a cycle, each segment driving the next. Loops in physical space have a structurally distinguished feature: a **bottleneck**<sup>17</sup>, the minimum value of information-carrying capacity (bandwidth) along the loop. Bandwidth along a loop is generically not uniform — there is a value at which it is minimized — and any surface that cuts the loop at this minimum-bandwidth value is a *bottleneck surface* of the loop. There may be many such surfaces.

A **constructive agent**<sup>18</sup> is the finite region enclosed by a bottleneck surface of minimum area. Two criteria, not one: the surface cuts the loop at minimum bandwidth (selecting the bottleneck value), and among all surfaces doing so, has minimum area (selecting where along the bottleneck stretch to localize the cut). The first criterion alone does not pick out an agent — many surfaces realize the minimum bandwidth, and among them many enclose finite regions of various sizes. The second criterion forces the cut to lie at the geometrically narrowest place along the loop, where the boundary is empirically familiar: cell membranes, skin, the surfaces of organisms. The agent is what these surfaces enclose.

The agent's existence is not prior to the loop — the agent is what the loop's bottleneck-and-minimum-area cut bounds. Where there is no loop, there is no agent.

The boundary may be sharp or a band. It is sharp when the minimum-bandwidth, minimum-area cut is unique — a single distinguished surface, as in a cell membrane or an animal's skin. It is a band when multiple surfaces achieve both criteria — a bacterial colony, or a cloud of correlated computational processes, where several candidate surfaces are equally minimal in both bandwidth and area. The band is the union of these equally-minimal surfaces, not a fuzzy version of a sharp boundary. Both cases satisfy the definition; the framework derives different consequences for each.

By §3.1.3 the physical space agents inhabit is unboundedly large, so the loop's spatial realization, when cut by the agent's boundary, partitions into a finite region (the agent) and an unbounded region (the environment). This finite-versus-unbounded asymmetry is what makes the framework's downstream derivations possible — most directly the termination derivation in §3.2.1.

*The loop's continuation requirement.* A feedback loop continues only if information continues to flow around the cycle. If the agent's contribution becomes fully predictable from the environment's side — if the agent's outputs add nothing the environment doesn't already anticipate — then the agent's segment of the loop carries no new information. The cycle ceases to circulate; the loop, as a loop, ends. From the agent's side, this is termination: not destruction of substrate, but loss of the agent's role in a loop that has ceased to be a loop.

This reframes what *survival* means for a constructive agent. Survival is the loop's continued circulation through the agent's region, which requires the agent to maintain outward informativeness — surprisal exerted on the environment. Reduced surprisal received from the environment may be a side effect of the agent's modeling activity, but it is not the survival target. The survival target is the agent's continued contribution to information flow around the cycle.

*Inversion of the Free Energy Principle.* The standard statement of the **Free Energy Principle**<sup>19</sup> proposes that agents act to minimize the surprise they receive from their environment. This document's framework inverts the optimization target: agents act to maintain or increase the surprise they exert on the environment, because outward informativeness is what keeps the loop circulating, and loop circulation is what the agent's existence consists of.

The Dark Room paradox follows directly. An agent that minimizes its outward surprisal — that produces only outputs the environment can already predict — has by hypothesis ceased contributing information to the loop. The loop short-circuits around it. Termination is not a contingent failure of such an agent; it is what "minimum outward surprisal" means.

The minimization of received surprise, where it occurs, follows from the agent's modeling of its environment: a well-modeled environment produces fewer surprises to the agent. But this is downstream of, not constitutive of, what the agent is doing. Whether the inversion is correct is one of the things the rest of the document argues; for now, it is the framework's optimization stance, and it follows from loop-priority rather than being asserted independently.

### 2.2 The Predictive Claim

The most directly testable empirical commitment of this document, stated as falsifiably as we can manage:
...
*A note on placement.* The claim above is logically a consequence of the broader central claim stated in §2.4 (that all §1.1 properties follow from foundation plus agent definition); its derivation through cognitive sacrifice accumulation is given in §3.2.11–§3.2.13. We state it here, before the central claim and before its derivation, because it is the most directly testable empirical prediction of the framework and serves as the base for understanding how the theory relates to empirical observation. Logical priority belongs to §2.4; presentation priority belongs here.
...
The empirical layer therefore has two timescales. Structural and innate-state properties identify agency at any developmental phase including agent formation: a newborn satisfies the constructive agent definition the moment its loop closes, by virtue of the substrate-encoded seed (§3.2.2) and its structural inventory, even though it will not exhibit much behavioral evidence of agency for some time. Behavioral and learned-state properties are the rising-then-falling indicators the predictive claim concerns; they are weak in early development (before $\mathcal{K}$ has accumulated), strong in maturity, and decline in the termination phase. Both timescales are part of the framework's empirical content, and identifying an agent depends on which timescale's evidence is available. Newborns are agents on structural-and-innate-state grounds; the absence of strong behavioral evidence is not evidence of absent agency, but evidence that behavioral observation isn't yet a useful identification channel for that agent at that developmental phase.

The bidirectional thesis (§2.1) — that the empirical and constructive definitions coincide — therefore operates differently across the two property types. Structural and innate-state properties bidirectionally connect to the constructive agent definition at all phases. Behavioral and learned-state properties bidirectionally connect only in the developed and mature phases when these properties have had time to accumulate. The predictive claim of this section is about the second bidirectional connection's time profile; the first is implicit in the structural inventory of §1.1 and is what licenses agent identification in early development.

The predictive claim's informativeness varies across the spectrum of agents. Spectrum position has a structural correlate: it tracks the ratio of seed-component groundedness $\mathcal{G}(M^{\text{phys}})$ to mature total groundedness $\mathcal{G}(M_{\text{mature}})$ (per §3.2.2 and §3.2.6 — the seed sits in $A_{\text{phys}}$, the modifiable component in $A_M$). For agents heavily seed-dominated — the RNA viroid is the clearest case in §1.2 — most of the agent's content is substrate-encoded at formation, $\mathcal{G}(M^{\text{phys}}) \approx \mathcal{G}(M_{\text{mature}})$, and behavioral and learned-state properties accumulate barely or not at all during the lifespan. In the limit, $A_M$ is degenerate (empty or vanishingly small): the agent has no fast-update substrate to support modifiable content, and lives entirely on its seed. The claim's curve technically holds (the rising phase is real but vanishingly small; substrate degradation and termination are real) but its empirical bite is minimal. For learning-rich agents — the human is the clearest case — most of the agent's content is acquired through experience, $\mathcal{G}(M^{\text{phys}}) \ll \mathcal{G}(M_{\text{mature}})$, and behavioral and learned-state properties accumulate substantially across decades; the claim's curve is observable across the whole lifespan and substantial predictions follow. The brain (and analogous fast-update substrates in some lineages) is what makes large $A_M$ available — an evolutionary innovation that arises in lineages whose environments reward within-lifespan adaptation enough to justify the substrate investment. The framework's compression succeeds for both kinds of agent (foundation, agent definition, and substrate-encoded seed account for both), but the predictive claim's specific time profile is informative in proportion to where the agent sits on the spectrum from seed-dominated to learning-rich.

The claim is falsifiable in several ways. An agent of substantial seed quality ($\mathcal{G}(M^{\text{phys}})$ well above the loop-closure floor, with non-degenerate $A_M$ supporting modifiable content) in a sufficiently entropic<sup>20</sup> environment that ages without any rise in behavioral or learned-state property exhibition would count against it. The "substantial seed quality" qualifier matters: agents with minimal seed and degenerate $A_M$ (the viroid case) are not expected to show a rising curve and don't falsify the claim by failing to show one; the agents that do put the claim at risk are those whose substrate could support trajectory traversal but who fail to traverse it. A class of agents that show no terminal-phase decline despite substrate degradation would count against the claim. A reliable pattern of property exhibition that doesn't track time at all — that emerges fully formed at any age — would count against it. The pattern the claim predicts is well-documented across the eight classes in §1.2 (development → maturity → senescence) for agents where the curve is informative, and specific quantitative tests can be designed for individual species at known spectrum positions.
...
The predictive claim above states what is observable. This section states why the observation should hold, by bridging clock time to a quantity the framework derives.
...
The document's overall thesis, stated as the framework's central commitment:

**Claim.** All directly observable<sup>2</sup> behavioral<sup>5</sup> and learned-state<sup>4</sup> properties from §1.1 — the empirical regularities of agents — follow as consequences of two things together: the constructive agent<sup>18</sup> definition of §2.1, and the established physical bounds (thermodynamics, finite information density, finite processing speed). No additional structure about agents is required.

This is a compression in the same sense thermodynamics compressed many heat-related observations to a few laws — and we will refer to it as the framework's *compression* throughout. Many specific regularities reduce to a small foundation. The remainder of the document works out the compression: which properties follow from which combinations of definition and physics, what the chains of inference look like, where the claims hold strictly versus where they hold on average across populations of agents, and where auxiliary assumptions would be needed to extend the framework further.

The predictive claim of §2.2 — property exhibition rising and falling with time — is the integrated empirical signature of this compression. If the compression holds, the pattern follows. If the pattern systematically fails, the compression is wrong.
...
The claim is derivable from the framework's deductive content — see §4.1.1 for the derivation. It is consistent with the predictive claim of §2.2 but stronger: it commits to ordered windows rather than just monotonic accumulation. The strong form sometimes encountered in developmental literature — total ordering across all properties — is *not* derivable (and probably wrong as stated, since different agent classes show different total orderings consistent with the same partial order). The derivable form is the partial-ordering claim.

*A note on physical development.* The developmental windows discussed above are *cognitive* windows: when properties become acquirable based on what content has accumulated. There is a separate dimension the framework currently doesn't derive: *physical development* of the agent's substrate during the developmental phase. For agents whose mature operational substrate is much larger than their seed — biological agents whose seed is encoded in DNA and whose mature body and nervous system must be physically constructed from the seed's instructions — the developmental phase is also a phase of substrate construction. At $t_0$, $A_{\text{phys}}$ contains the seed but $A_M$ may be mostly unbuilt; the brain or analogous fast-update substrate is constructed over time, and only becomes operational for substantial logging once construction reaches the relevant capacity. The rising phase of property exhibition we observe in such agents is the joint product of two processes: substrate construction (physical development) and logged content accumulation (cognitive development). Early in the developmental phase, substrate construction dominates; later, logged accumulation dominates. The framework operates on cognitive operations on existing substrate; it does not derive the physical-development dynamics that build the substrate. Developmental dynamics are substrate- and lineage-specific (mammalian neural development is unlike plant tissue development is unlike artificial-agent training-as-development), and the framework operates above substrate-specifics. What the framework does claim is that *some* developmental process must occur for agents whose mature operational substrate substantially exceeds their seed; the specific dynamics of that process are flagged for future work (§5.2). This dimension does not affect the compression claim of §2.4, which operates on cognitive dynamics given whatever substrate is currently constructed; it does affect how the predictive claim's curve in §2.2 should be interpreted for learning-rich agents in their early developmental phases, where substrate-construction confounds with cognitive accumulation in producing the observable trajectory.
...
*A note on the strength of derivations.* The chapter's results are not all of the same strength. Three levels are distinguished. *Derived* results follow theorem-like from the foundation by valid inference at each step — examples include §3.2.1 bandwidth finiteness, §3.2.7 forgetting, §3.2.8 habituation, §3.2.10 layered self-model and optimization gap, §3.2.11 cognitive sacrifice accumulation, and §3.2.15 internal-grounding bound. *Forced under stated assumptions* results follow from the foundation given additional premises that are made explicit at the point of use — most of §3.2 is at this level, including world-model necessity (§3.2.2), inference class membership (§3.2.3), substrate trade-off (§3.2.6), self-model as content (§3.2.9), and termination signatures (§3.2.13). *Plausibility-argued* results are the framework's most likely conclusions given the foundation plus reasonable assumptions, but where alternatives aren't strictly excluded — §3.2.5's specific form-constraints on $V^{\text{cont}}$ and §3.2.14's three-step decomposition of inference are the main cases. The chapter's prose uses "derives" for the first level, "forces" or "is forced" with stated assumptions for the second, and "plausibly follows" or "the framework argues" for the third. Where the chapter claims results follow from "the foundation," it means the foundation plus the level-appropriate assumptions, not theorem-like consequence.

*A note on class-membership tests.* Several derivations in §3.2 (inference §3.2.3, logging §3.2.4, partition §3.2.6, memory hierarchy and working memory §3.2.7) characterize a *class* of admissible mechanisms rather than a specific algorithm. Class membership is the falsifiable claim; specific class members are substrate- and architecture-dependent. The class definitions are paired with operational tests for membership at the end of each section. The test *structure* is universal — the same procedure applies to any candidate mechanism — but specific numerical *thresholds* are substrate- and task-dependent. What counts as "measurable change," "different output," or "disruption" depends on the agent's substrate and input regime; the test telling you whether to look for these is the same regardless. This combination — universal test structure with substrate-dependent thresholds — is what makes the framework falsifiable without overcommitting to specific implementations.
...
By §2.1's loop-continuation requirement, the agent's contribution to the loop must remain non-redundant — its outputs must carry information into the cycle that the environment doesn't already have. The agent must therefore contain something that lets it produce such outputs systematically rather than by chance.

The argument: an agent's outputs in the absence of any structure for predicting environmental responses would be effectively random from the environment's standpoint. The environment's entropy of the agent's outputs would be high; outward informativeness would persist. But this scenario also means the agent is producing outputs without sensitivity to environmental state — a constraint the loop definition rules out, since a loop requires the agent's outputs to depend on inputs.

So the agent's outputs depend on inputs. The dependence must be such that outward informativeness is sustained. Random agent-output mappings, given input-dependence, would over time produce environments that adapt and predict — accelerating termination per §3.2.1. Therefore the agent's input-output mapping must reflect *something about the environment* — must contain regularities that produce environment-informative outputs systematically rather than by chance.
...
A note on what $M$ is and isn't. $M$ is a functional structure, not a commitment to representational content in any strong philosophical sense. The framework's claim is that some persistent agent-internal structure must exist that (a) varies with the agent's history, (b) correlates with environmental patterns, and (c) is causally efficacious in producing outputs via the inference mechanism of §3.2.3. Whether one describes this structure as "representational," as "dynamical coupling state," as "sensorimotor attunement," or in some other vocabulary is a labeling choice the framework does not arbitrate. What the framework's derivations require is the structure's existence and its functional role; they do not require any specific philosophical interpretation of its content. Positions that deny only the label (some readings of enactivism, dynamical-systems cognition, ecological psychology) are compatible with the framework. Positions that deny the structure itself — that hold cognition to be constituted entirely by ongoing dynamical activity with no role for any persistent internal state — are ruled out, but empirically rather than by stipulation: the behavioral properties documented in §1.1 (habituation, conditional response, novelty discrimination) require persistent state that varies with history, and the framework's derivations are downstream of that empirical fact.

The world model is non-degenerate from agent formation onward, not built from zero. By §2.1, loops form only when the materials carrying the loop have sufficient substrate-encoded structure to sustain a self-closing channel of information flow at all; this minimum is whatever the local thermodynamic conditions require for loop closure. Below it, no loop forms and no agent exists. At agent formation, this minimum is already encoded in the substrate — the materials' physical-chemical properties carry whatever model content is necessary for the loop to close as a loop. Logging-as-learning (§3.2.4) extends $M$ from this seed; it does not create $M$ from nothing. Empirically, this is what every agent in §1.2 demonstrates: the viroid's fold pattern, the chemoreceptor proteins of bacteria, the trigger geometry of *Mimosa*, the body plan of vertebrates — all present at agent formation as substrate-encoded model content, the seed on which lifetime logging operates.

The world model is distributed across both sub-regions of the partition derived in §3.2.6. The seed lives in $A_{\text{phys}}$ — DNA in biological agents, hardcoded weights in some artificial agents, design-frozen content otherwise — by virtue of the energy-tolerant durable substrate $A_{\text{phys}}$ provides. Logging-acquired content lives in $A_M$ — synaptic and similar substrates in biological agents, trainable weights and activations in artificial agents — by virtue of the fast-update information-dense substrate $A_M$ provides. The agent's total world model at time $t$ is the composition: $M_t = M^{\text{phys}} \cup M^{\text{mod}}_t$, where $M^{\text{phys}}$ is set at formation and persists, and $M^{\text{mod}}_t$ starts near-empty and grows via logging. Seed quality is $\mathcal{G}(M^{\text{phys}})$ — how well the durable component correlates with the environment the agent encounters. Seed quality varies across agents and lineages: minimum-grounded just-above-loop-closure for some, substantially refined by evolutionary or design history for others. The framework operates on $M_t$ within the lifespan; the framework does not derive how $M^{\text{phys}}$ got to its initial value, only that some $M^{\text{phys}}$ with $\mathcal{G}(M^{\text{phys}}) \geq \mathcal{G}_{\min} > 0$ is present at $t_0$.

#### 3.2.3 Inference and trajectory reuse
...
What's forced by the foundation is not a specific inference algorithm but a class of mechanisms satisfying certain properties. The framework derives the class; specific algorithms (exact-match nearest-neighbor, kernel-weighted similarity, attention over key-value memory, embedding-proximity search, Hopfield retrieval, holographic associative memory, modern retrieval-augmented architectures) are class members. What's binding is the class membership; the choice among members is substrate- and architecture-dependent.

The class properties, each derivable from the foundation:

- *Trajectory dependence.* The mechanism's outputs depend on the agent's past trajectory record. By §3.1.4, the agent has bounded capacity; the model's content beyond the substrate-encoded seed (§3.2.2) cannot be present in advance and must be acquired through the agent's interactions. The only persistent record the agent has access to for this acquired component is its own past trajectory.

- *Context sensitivity.* Outputs depend on the current input window and the agent's current state in a way that uses the trajectory record's relevance to the current context. A mechanism that produced context-independent outputs from the record would fail to use the record's structure.

- *Similarity-based reuse.* The mechanism uses some notion of similarity, resemblance, or relevance between the current context and past trajectory regions to determine which past content informs the current output. Without a similarity criterion of some kind, the record's contents can't be selectively activated for the current context.

- *Bounded operation.* The mechanism's per-step computation is bounded by §3.1.4 capacity and §3.1.5 processing speed. It cannot exhaustively process the entire record on every step in unbounded time.

- *Forward projection.* The mechanism produces outputs that extend, continue, or respond based on the matched or relevant past content, rather than just retrieving it.

The canonical exemplar — and the framework's working reference throughout subsequent chapters — is autocorrelation against logged trajectory: the current input window is matched against past windows in the log, the matched template is projected forward, and the projection is the agent's output. This is one class member, chosen for being the simplest case satisfying the properties above. Other class members produce qualitatively similar behavior with different implementation specifics.

Several alternative mechanisms are *not* class members. A pure parametric model that compresses experience into parameters and discards the trajectory record fails Trajectory dependence (the record is gone). Random output generation fails Trajectory dependence. Hardcoded input-output maps fail Trajectory dependence. A mechanism that depends on context but ignores similarity to past — for example, always producing the chronologically last output regardless of current input — fails Similarity-based reuse. Each of these alternatives is excluded by class properties, not by parsimony or convenience.

Compressed parametric models that *maintain access to representative trajectory content* (such as gradient-trained networks paired with experience replay buffers, or attention-based architectures over key-value stores) *are* class members. They preserve the class properties even though their implementation differs from straightforward autocorrelation. The framework permits them because they satisfy the constraints; what they substitute is the algorithmic specifics, not the structural requirements.

The class characterization is what's empirically testable. Any agent we observe whose inference mechanism falls outside the class would falsify the framework. Any agent whose inference mechanism is a class member — regardless of which member — is consistent with the framework's derivation.

*Operational tests for class membership.* The properties P1–P5 are qualitative; specific numerical thresholds depend on substrate. What's substrate-independent is the test structure: each property admits an operational test that any candidate mechanism can be evaluated against. P1 trajectory dependence: ablate the trajectory record (zero-out the replay buffer, ablate the key-value store, otherwise remove logged content) and re-run the mechanism on inputs from the agent's typical input regime. If outputs are unchanged or near-unchanged, the mechanism fails P1 — its outputs were determined by compressed parameters alone, not by trajectory access. Measurable change is required for P1; small effects from large records do not suffice. P2 context sensitivity: vary the recent input window while holding everything else constant; the mechanism must produce different outputs. P3 similarity-based reuse: replace the most-similar logged content with arbitrary content (preserving the rest of the log); if outputs are unchanged, the mechanism fails P3 — outputs were determined by aggregate features rather than selective similarity. This excludes pure linear regression on aggregate features while including kernel methods, attention, retrieval, and similar mechanisms. P4 bounded operation: per-step computation must scale at most linearly with bounded constants in log size; mechanisms with worse scaling fail P4 as the log grows even if they satisfy P4 at some specific log size. P5 forward projection: the output must be in the image of trajectory-derived projection. The threshold for "measurable change," "different output," and similar is substrate- and task-dependent; the test structure is universal.

#### 3.2.4 Logging as learning

If inference is in the trajectory-reuse class (§3.2.3), the record on which inference operates must exist and be maintained. Sensing produces internal state changes (input transduction, signal propagation, downstream processing). Acting produces internal state changes too (output formation, motor commands). If these state changes are recorded persistently and ordered, the record exists automatically — sensing and acting both contribute to it as side effects of normal operation.

What's forced is not a specific recording mechanism but a class of mechanisms satisfying the properties that §3.2.3 inference requires. We call any such mechanism **logging**<sup>25</sup>; the canonical exemplar is the persistent ordered record of internal state changes during sensing and acting, accessed directly by inference.

The class properties for logging mechanisms:

- *Persistence.* The record must persist across operations that read from it — at minimum from the time of the recorded event through the time the inference mechanism uses it.
- *Sufficient temporal structure.* The record must preserve enough temporal ordering or context information for the §3.2.3 class member to operate. Strict total ordering is sufficient but not necessary; partial ordering with timestamps, or local context preservation, also suffice.
- *Substrate-supported writability.* The substrate must support recording at the rate sensing and acting occur, bounded by §3.1.2 finite power and §3.1.4 finite capacity. Recording rates below sensing rates would lose information needed for inference.
- *Read-accessibility by inference.* The record must be accessible to whichever class member from §3.2.3 implements inference, in the format that class member operates on.

Class members include synaptic weight changes (with timing-dependent plasticity preserving order), DNA methylation patterns and other epigenetic marks, calcium dynamics in cellular systems, distributed activation patterns in neural networks, explicit episodic memory stores, content-addressable memory traces, log-structured storage in engineered systems, and modern key-value memory architectures. Each maintains a persistent record sufficient for some class-member inference mechanism to operate.

Logging is not a separate cognitive operation added on top of inference; it is the form taken by the agent's operations whenever their state changes are persistent in a way the inference class member can use. There is no separate learning rule that updates parameters independently of the record; the trajectory accumulates in whatever class-member form, and inference draws on it. New experiences become available to influence future inference simply by being logged. The agent learns by living — extending the modifiable component $M^{\text{mod}}_t$ in $A_M$ (per §3.2.6) through accumulating $\mathcal{K}$, while the durable seed component $M^{\text{phys}}$ in $A_{\text{phys}}$ stays as set at formation. Logging operates on $A_M$-content exclusively; the seed is read by inference but does not update during the lifespan. For agents with degenerate $A_M$ (no fast-update substrate, all world-model content in the seed), logging is itself degenerate: there is nothing to log to, and the agent operates entirely on its seed without within-lifespan modification.

The substrate's writability at sensing rates is therefore implicit in any agent satisfying §3.2.3. Substrates that cannot record fast enough to capture sensing-rate events fail to support class-member inference, and agents on such substrates cannot satisfy the framework's derivations.

*Operational tests for class membership.* L1 persistence: probe logged content at increasing time intervals after the recorded event; L1 is satisfied if probes successfully retrieve content across the timescales the agent's inference mechanism uses, failed if content decays faster than inference accesses it. L2 sufficient temporal structure: the bag-of-experiences case (no temporal ordering at all) fails L2 for any inference mechanism that uses sequence context, which most do; an agent's L2 is checked against the temporal-structure requirements of its specific inference class member, and what's "sufficient" varies by class member. The test: does the inference mechanism access ordering information when present, and does its output change if ordering is shuffled? If yes, the log must preserve ordering; if no, the log doesn't need ordering. L3 substrate-supported writability: measure the recording rate against the sensing/acting event rate during typical operation; selective logging fails L3 if the unrecorded fraction systematically loses inputs that the inference mechanism's outputs depend on. L4 read-accessibility: the format of the logged record must match what the inference mechanism operates on; format mismatches that prevent reads on otherwise-present content fail L4. As with §3.2.3, specific numerical thresholds depend on substrate; the test structure is universal.
...
A note on what this claim is and isn't. The rationalizability move is a structural-existence claim, not a behavioral-fit claim. The framework asserts that given the agent's complete internal state — model contents, working memory, current cognitive load, all of $S_A$ — choice is deterministic and consistent, so a utility function exists at this level of description. The framework does *not* assert that agents satisfy classical rationality axioms in their observable choice behavior. Empirical violations of transitivity, context independence, and consistency are well-documented across humans, other animals, and bounded-rationality models (Simon, Tversky-Kahneman, Gigerenzer); the framework is compatible with all of them. Such violations reflect either (i) incomplete external observation of the agent's internal state — different choices under "the same conditions" actually occurred under different internal states — or (ii) the optimization gap of §3.2.10, where the agent optimizes its drifted representation $\widehat{V}^{\text{cont}}$ rather than the underlying $V^{\text{cont}}$. Neither contradicts the structural existence claim. Bounded-rationality phenomena are what the framework's optimization looks like under external observation; the framework's derivation operates at the level where the optimization is well-defined.
...
What's forced for any agent facing both informational and energetic inputs of significant magnitude is *a partition* of the agent's region into substrate-tolerant and information-rich sub-regions, joined by some internal interface or interfaces. The class of admissible partition geometries is broad; what's binding is the partition's existence and its respect of the substrate trade-off.

The class properties:

- *Substrate differentiation.* The agent's region $A$ partitions into at least two sub-regions, at least one with energy-tolerant substrate (call it $A_{\text{phys}}$, the **physical sub-region**<sup>29</sup>) and at least one with information-rich substrate (call it $A_M$, the **mental sub-region**<sup>28</sup>).
- *Internal interface(s).* The sub-regions are joined by at least one interface across which information flows. The interface has its own bandwidth, tunable independently from the external boundary's bandwidth.
- *External shielding.* High-energy-flux inputs from the environment are absorbed primarily by $A_{\text{phys}}$, with information extracted and forwarded across the internal interface to $A_M$. The mental sub-region is not directly exposed to the energy-bearing component of inputs.
- *Coupled loops.* Any feedback loop with the environment passes through both sub-regions: external signals enter through $A_{\text{phys}}$, are processed in $A_M$, and outputs return through $A_{\text{phys}}$ to the environment.

The canonical exemplar — and the framework's working reference — is the **figure-8 structure**<sup>30</sup>: a binary partition with one external loop (environment ↔ $A_{\text{phys}}$) and one internal loop ($A_{\text{phys}}$ ↔ $A_M$), joined at the physical sub-region.

Other class members include:

- *Sharp binary partition* — figure-8 with discrete, well-localized internal interface (vertebrate skull/brain split, eukaryotic nucleus/cytoplasm distinction).
- *Gradient transition* — physical and mental sub-regions joined through a region of intermediate substrate properties rather than a sharp interface (bacterial subcellular organization, plant tissue gradients).
- *Distributed mental sub-region* — multiple information-rich sub-regions joined through several internal interfaces to one or more physical sub-regions (cephalopod arm-and-central-brain neural distribution).
- *Hierarchical partition* — partition applied recursively, with the mental sub-region itself partitioned into further substrate-tolerant and information-rich layers (cell within tissue within organ; multi-tier engineered systems).
- *Multi-tier shielding* — multiple physical sub-regions of decreasing energy tolerance, with the most information-rich sub-region most deeply shielded.

Each member satisfies the class properties; choice among members is substrate- and topology-dependent. The bandwidths of the internal interface(s) typically exceed those of the external boundary, since the internal interface is shielded from energy-bearing inputs and can be tuned for high information density.

*The partition's dual purpose: input handling and content-temporal-structure.* The substrate trade-off as derived above forces the partition to handle the two-dimensional input structure (information rate and energy flux) — $A_{\text{phys}}$ absorbs energy-bearing inputs, $A_M$ processes information-rich ones. But the same trade-off also governs *content storage*. Some world-model content needs to be durable: preserved across the agent's lifespan, robust to perturbation, copyable to descendants where applicable. Durability requires a substrate that resists modification — exactly the property $A_{\text{phys}}$ has by virtue of energy-tolerance. Other world-model content needs to be modifiable at logging rates: accessible and rewritable as inputs arrive. This requires a substrate whose state can change quickly — what $A_M$ provides by virtue of being information-dense. The partition therefore has dual purpose: it handles the two-dimensional input structure *and* it supports temporally-distinct content storage. World-model content is distributed across both sub-regions: durable content (the seed, encoded in DNA for biological agents, hardcoded weights for some artificial agents, design-frozen content otherwise) sits in $A_{\text{phys}}$; modifiable content (synaptic and similar in biological agents, trainable weights and activation patterns in artificial agents) sits in $A_M$. Both are part of the agent's $M_t$ at any time; together they constitute the world model the framework derives in §3.2.2.

*The degenerate-$A_M$ case.* For agents whose entire world model is durable seed content, $A_M$ may be empty or vanishingly small. The RNA viroid is the limit case: its fold pattern, catalytic activity, and binding behavior are all sequence-encoded; no within-lifespan modification of inference-supporting content occurs; nothing accumulates through logging. For such agents the partition is degenerate — $A_M$ is empty and all framework machinery operating on $A_M$ (working memory, episodic-to-semantic consolidation, and similar derivations downstream of §3.2.7) is vacuous because there is no modifiable content for it to operate on. This is consistent with what we observe empirically across §1.2's classes: the viroid exhibits very few of §1.1's behavioral and learned-state properties, while the human exhibits many. The framework's machinery scales with $A_M$ size; agents with degenerate $A_M$ live entirely on their seeds. The brain (and analogous fast-update substrates in some lineages) is what scales $A_M$ to support substantial logging-as-learning — an evolutionary innovation that arises in lineages whose environments reward within-lifespan adaptation enough to justify the substrate investment.

*Operational tests for class membership.* PT1 substrate differentiation and PT3 external shielding are tested jointly: under the agent's actual input regime, do high-energy inputs reach $A_M$ in ways that disrupt its function? If yes, $A_{\text{phys}}$ is not actually shielding $A_M$ from energy-bearing inputs, and the partition fails. (For agents with degenerate $A_M$, this test is vacuous; the relevant test is whether high-energy inputs disrupt $A_{\text{phys}}$'s seed content, which they typically don't because durable substrate is energy-tolerant by construction.) The threshold for "disruption" is substrate-dependent, but the test is operational: measure each sub-region's function under the agent's actual high-energy input distribution and check whether function degrades. PT1 also requires that $A_M$, when non-degenerate, supports the information density required by the inference mechanism — measured as whether $A_M$'s state space has the resolution and addressability the agent's modifiable-content inference operations need. PT2 internal interface(s): the internal interface's bandwidth must be tunable independently of the external boundary's bandwidth, testable by checking whether the agent can adjust internal information flow without correspondingly adjusting external transduction. PT4 coupled loops: any feedback loop with $E$ must pass through both sub-regions when the agent has modifiable content; for degenerate-$A_M$ agents, loops pass through $A_{\text{phys}}$ alone with the seed serving as the sole world-model content. The framework permits inference operations that read from $A_{\text{phys}}$-content (seed-driven inference is exactly this); what's ruled out is inference that bypasses both sub-regions' world-model content entirely.

#### 3.2.7 Forgetting, memory hierarchy, and working memory

Three memory-related consequences from finite capacity (§3.1.4) plus continuous logging (§3.2.4) plus variable deadlines (§3.1.5) plus inference cost.
...
*Memory hierarchy.* The §3.2.3 inference mechanism's lookup cost scales with stored entropy; match quality scales with stored entropy in the opposite sense. A single representation at any fixed entropy level fails some deadline regime: too slow when deadlines are tight, more precise than needed when deadlines are loose. What's forced is a partition of the log into at least two levels with different cost-quality trade-offs. The class of admissible memory hierarchies includes:

- *Two-level partition* — a high-entropy detailed level (slow, precise) and a low-entropy compressed level (fast, coarse). The canonical exemplar, with the two levels corresponding to **episodic memory**<sup>31</sup> and **semantic memory**<sup>32</sup>.
- *Multi-level hierarchy* — three or more levels with progressively coarser representations and faster lookup, producing a graded structure across deadline regimes.
- *Continuous gradient* — a continuum of representations at different entropy levels rather than discrete partitions, with lookup operations selecting depth based on deadline.

What's binding is that *at least two levels* exist. A conversion process between levels (high-entropy entries getting compressed into lower-entropy summaries) is required for any class member.

*Working memory.* Tight-deadline decisions cannot search the full log. They cannot use only semantic memory either, when fine matches are needed. What's forced is some pre-loaded fast-access subset for tight-deadline operations. The class of admissible fast-access mechanisms includes:

- *Explicit working memory buffer* — a small subset of episodic content held "warm" in the inference mechanism's fast-access pathway. The canonical exemplar; this is **working memory**<sup>33</sup>.
- *Sustained activation patterns* — neurally-implemented persistent firing keeping recent inputs accessible without explicit buffering.
- *Attention-based gating* — selective amplification of subset of memory contents through attention mechanisms, functionally equivalent to "warm" subsetting.
- *Cache hierarchies* — multi-level fast-access stores in engineered systems.

What's binding is that *some* fast-access mechanism exists. Its capacity is bounded by available internal-interface bandwidth divided by per-item maintenance cost, regardless of which class member implements it.

*Operational tests for class membership.* For the memory hierarchy: do the levels have measurably different cost-quality trade-offs under the agent's actual workload? Test by running typical inference operations and measuring lookup latency and match quality at each putative level. If multiple stores collapse to the same trade-off (identical access times and entropy levels), the agent has effectively one level and fails the partition requirement. The conversion process between levels is testable by tracing whether content at higher-entropy levels measurably influences content at lower-entropy levels over time — without conversion, the levels are independent and don't satisfy the framework's requirement of a consolidated hierarchy. For working memory: is some subset of memory content reliably faster to access than the bulk store, and does this subset change over time as the agent's context changes? Test by measuring access latency for content versus log-membership; if the fast-access subset is fixed (always the same items) or doesn't exist (uniform access times), the working memory class membership fails.
...
Why does cognitive sacrifice drive behavioral and learned-state property development? This subchapter completes the predictive claim of §2.2 by showing the second half of the bridge.
...
Composing §3.2.11 + §3.2.12: clock time correlates with $\mathcal{K}$, and $\mathcal{K}$ correlates with property exhibition probability. The predictive claim of §2.2 is established for the rising phase. The falling phase comes next.
...
Why inference, in any class member from §3.2.3, plausibly decomposes into matching, interpretation, and projection.

The inference mechanism finds matches in $\ell$ for the current context — by autocorrelation, by attention, by embedding-proximity, or whatever the class member's specific algorithm. Once a match is found, the mechanism projects forward: the immediate continuation or relevant aspect of the matched content in $\ell$ becomes the predicted output.

But projection is not free. The same matched content can be relevant in different ways depending on which features of it matter for the current context. A matched window where the agent succeeded in feeding can be projected forward as "approach" (if the framing is action-oriented) or as "ingestion" (if the framing is consumption-oriented) or as "satiation" (if the framing is state-oriented). Each is a valid forward-projection of the same match, but they yield different outputs.
...
Interpretation has its own cost (separate from matching) and its own failure mode (the right match interpreted under the wrong frame produces wrong projection). The existence of projection ambiguity is forced for any class member admitting multiple valid forward-projections from a single match — which all class members do, since similarity-based reuse (P3) plus forward projection (P5) jointly admit projection ambiguity unless the class member happens to have a degenerate frame structure. The decomposition into matching → interpretation → projection as three architecturally distinct steps is one way to handle the ambiguity, and the framework's working reference; it plausibly follows from the ambiguity's existence and is convenient for analyzing what class members must do, but it is not strictly forced — different class members may resolve the ambiguity within Match or Project rather than as a separate Interpret step (attention mechanisms through learned key-value weighting, kernel methods through kernel choice, nearest-neighbor through tie-breaking rules). What is forced is the ambiguity; the three-step decomposition is the framework's plausibility-argued account of how it gets resolved.

**Frames**<sup>41</sup> drift along with other interpretive content (per §3.2.10), so frame stability is environmental rather than purely internal, and frame selection in cases of multiple applicable frames produces a regress (a frame is needed to choose among frames). These follow as further consequences of the decomposition.
...
Existence-level deductions:
    ...
    3.2.3 Inference and trajectory reuse ← 3.2.2, 3.1.4
    3.2.4 Logging as learning ← 3.2.3, 3.1.2
    3.2.5 Goal function ← 3.2.2, 3.2.4, decision necessity
    3.2.6 Substrate trade-off and partition class ← 3.1.1, 3.1.4, agent definition
...
This chapter audits the deduction of Chapter 3 against the empirical layer of Chapter 1. Six subsections walk through the results: directly observable properties accounted for by the framework (§4.1); substrate-specific particulars that lie below the framework's level of operation by design (§4.2); implied non-observable structure surfaced along the way (§4.3); novel observable predictions that emerge from the framework (§4.4); literature claims that the framework's derivations falsify (§4.5); and operationalization of the framework's theoretical constructs through observable proxies (§4.6).

The framework's central claim from §2.4 — that all behavioral and learned-state properties from §1.1 follow from the constructive agent definition combined with established physical bounds — is honored: §4.1 covers all directly observable properties at the levels of strength named in the methodological note at the start of Chapter 3 (most by derivation, the rest as forced consequences under stated assumptions, with a small number plausibility-argued), with substrate-specific instantiations belonging below the framework's level of operation (§4.2). The framework's derivations therefore stand as falsifications of contradictory literature claims (§4.5), without escape through "the framework is missing something."

### 4.1 Properties Accounted For

The properties from §1.1 that follow from Chapter 3's deductions, with the route noted briefly. Each "Derived from..." pointer below indicates the §3.2 subchapter that accounts for the property; the strength of that account (derivation, forced consequence under stated assumptions, or plausibility-argued) is the strength named in the methodological note at the start of Chapter 3 for that subchapter, not necessarily theorem-like. The list is grouped by property type.

**Structural properties:**

- *Physical boundary distinguishable from environment.* Derived from the agent definition (§2.1) plus §3.2.1: the agent's region is enclosed by a minimum-area bottleneck cut of the feedback loop, which is by construction a distinct surface separating agent from environment.
...
- **The inference mechanism class.** §3.2.3 derives that inference belongs to the trajectory-reuse class — mechanisms satisfying trajectory dependence, context sensitivity, similarity-based reuse, bounded operation, and forward projection. The class itself is the framework's account of how outputs are produced; its specific instantiation in any given agent (autocorrelation, attention, retrieval-augmented architectures, others) is a class member. The class is structural; specific class members are not directly observable, and identifying which class member implements inference in a given agent is a substrate-and-architecture question.
...
With §4.1 now covering all directly observable properties from §1.1 (at the levels of strength named in the methodological note at the start of Chapter 3 — derivation, forced consequence under stated assumptions, or plausibility-argued; substrate-specific instantiations belong below the framework's level of operation per §4.2), the framework's central claim is binding. The framework accounts for the full empirical layer of agency from physical foundations plus the constructive agent definition. Its implied non-observable structure (§4.3) and the derivations themselves therefore contradict several positions held in the literature. Where these contradictions are substantive — not just differences of emphasis but logical incompatibilities — the framework's derivations stand as falsifications of those positions.
...
If the framework's derivations are correct — and the framework accounts for all the empirical regularities in §1.1, so the alternative is the framework being wrong about something the empirical layer already confirms — these literature claims are wrong as descriptions of agents.

**The Free Energy Principle's central claim — that agents minimize their own surprisal — is falsified.** The framework's central inversion (§2.1) derives, from loop-priority, that agents optimize the surprisal exerted on the environment, not received from it: outward informativeness is what keeps the loop circulating, and loop circulation is what the agent's existence consists of. The minimization of input surprisal is a side effect of environmental modeling, not the optimization target.

A defender of FEP may argue that the two formulations are dual — different vocabulary for the same underlying dynamics, since an agent that maintains outward surprisal will also tend to receive surprises it can resolve. This duality is what the falsification must address. Two empirical predictions distinguish the theories:

*Prediction A (perfect-model regime).* Consider an agent that has perfectly modeled its environment and faces no incoming surprise. FEP predicts this state is optimal — the agent has reached its target and should persist there indefinitely. Loop-priority predicts the opposite: a perfectly-modeled environment, under sustained interaction, has also accumulated a model of the agent; the agent's outputs become predictable from the environment's side; the loop short-circuits; the agent terminates. The two theories make opposite predictions about long-run viability under perfect mutual modeling. Empirically, biological agents under highly stable conditions do show degraded persistence — sensory deprivation, environmental enrichment effects on lifespan, the deleterious effects of monotonous environments on cognition. These observations are anomalies under FEP and consequences under loop-priority.

*Prediction B (output novelty regime).* In environments where the agent's model is already good, FEP predicts the agent should suppress output variability — fewer surprises to resolve means less drive toward exploration. Loop-priority predicts the opposite: agents in well-modeled environments should *introduce* novelty into their outputs to maintain outward informativeness as the environment learns them. Observable as exploratory behavior or output diversification in agents whose environments are stable and predictable. The pattern is documented across species (curiosity-driven behavior in animals with met needs, creative output in humans whose survival is secure) and is anomalous under FEP without auxiliary epistemic-foraging assumptions; loop-priority predicts it directly.

The Dark Room paradox is a special case of Prediction A. An agent in a dark room has zero outward informativeness; the loop short-circuits around it; the loop, as a loop, ends. FEP's prediction that such a state should be optimal is contradicted by the framework's derivation that it is what termination *means*. References that posit free-energy minimization as the agent's optimization target are falsified to the extent the framework's derivation holds and to the extent Predictions A and B are observed.

**Architectures positing separate self-model and environment-model are falsified.** §3.2.9 derives that the world model is a single conditional distribution over the joint state space $S_A \times S_E$, not a product of marginal models. Content separation between self and environment is permitted (the agent represents both); architectural separation — distinct modules for self-modeling vs. world-modeling, with their own update rules and independent operation — is not.

The falsification's targets are specific. Several modular cognitive architectures treat self-monitoring as a separate module from environment-modeling with distinct memory stores and processing pipelines (older ACT-R formulations and some SOAR variants are the cleanest cases; modern revisions in both traditions have generally moved toward integration but the architecturally-separate version persists in some implementations). Several AI alignment proposals advocate explicit "world model" and "self model" components as separable architectural units, often motivated by interpretability — a separable self-model is easier to inspect, the argument goes — and the framework rules against this design pattern. Some predictive-processing accounts in cognitive science treat interoceptive prediction (self-monitoring) as architecturally separate from exteroceptive prediction (environment-modeling), with distinct hierarchical streams that don't share representations; the framework rules these against. Note that some active-inference implementations also separate self-generative and environment-generative models even though the underlying FEP theory permits joint formulations — the implementations are falsified, the theory isn't.

The distinguishing prediction: if self and environment models were architecturally separate, an agent should be able to update one while the other remains unchanged — information specifically about the agent should update only the self-model, leaving environment-predictions untouched. The framework predicts this is impossible. Any update to one constituent of the joint distribution affects the other through their conditional dependencies: information about the agent's own properties also constrains what environments are consistent with those properties, and this propagates to environment-related predictions even when the input was nominally about the agent alone. Empirically testable: present an agent with information targeted at self-properties (the agent's reaction time, its accuracy, its internal state); measure whether predictions about the environment also change. Architecturally-separate-models predict no change in environment-predictions; joint-distribution predicts measurable correlated change. The pattern is consistent with documented effects in both human cognition (self-relevant information altering environmental judgments) and modern integrated AI architectures (where self-information propagates to non-self-related predictions through shared representations).

The contradiction is at the architectural level: the framework permits any decomposition of $M$ into content components, but rules out architectural decompositions that would let self and environment be modeled by independent systems.

**Pure offline parametric learning without inference-time trajectory access is falsified as a complete account of cognitive learning.** §3.2.3 derives that any inference mechanism must satisfy the trajectory-reuse class properties — trajectory dependence, context sensitivity, similarity-based reuse, bounded operation, forward projection. A mechanism that compresses experience entirely into parameters during an offline training phase and operates at inference time without any access to logged trajectory content fails P1 trajectory dependence: the operational test of §3.2.3 (ablate the trajectory record and re-run on typical inputs) trivially shows no change, since there is no trajectory record to ablate.

The falsification's scope is narrower than "gradient-based learning is falsified." Modern AI systems that combine gradient-trained parameters with inference-time mechanisms providing trajectory access — experience replay buffers, attention over key-value memory, retrieval-augmented generation, in-context learning over conversation history — *are* class members and are not falsified. What's ruled out is the strict version: cognitive learning fully captured by offline parameter updates, with the model at inference time treating each input as if it were the first. Theories that posit such purely-parametric inference-time mechanisms as a complete account of cognitive learning contradict the framework. Theories that posit gradient training plus inference-time trajectory access (the dominant pattern in contemporary machine learning and a documented pattern in biological learning systems via working memory, episodic recall, and similar) are consistent with the framework.

This falsification is significant despite its narrower scope. Several theoretical frameworks in computational neuroscience treat synaptic-weight changes as the complete substrate of learning, with inference being a function of weights alone — implying agents at inference time have no trajectory access beyond what's encoded in weights. The framework's derivation, if correct, says these treatments are incomplete as descriptions of biological agents and would fail as designs for artificial ones operating in environments richer than their training distribution.

**Reinforcement learning as a cognitive primitive distinct from learning-in-general is falsified.** §3.2.3 + §3.2.4 establish that learning happens through trajectory-reuse class members operating on logged content. Utility propagation — the assignment of utility values to past trajectory states based on subsequent outcomes — is materialization of cached values within a class member, not a distinct mechanism.

The falsification's target needs care, since "RL is falsified" would be both too broad and obviously wrong. RL as an algorithmic family — Q-learning, policy gradient, actor-critic, modern deep RL — works by mechanisms that satisfy the trajectory-reuse class properties: replay buffers (DQN), on-policy traces (REINFORCE), bootstrapping over remembered transitions (TD methods). These are class members and are not falsified. What is falsified is the stronger claim, found in some computational neuroscience and theoretical-cognition literature, that RL constitutes a *cognitive primitive* — a fundamental operation distinct in kind from associative, episodic, or declarative learning, with its own neural substrate operating by different principles. The dopamine-reward-prediction-error tradition in computational neuroscience (Schultz, Dayan, Montague, and successors) is the central case: RL is sometimes treated as a separate "system" that propagates values by mechanisms specific to itself, distinct from the brain's other learning machinery.

The distinguishing prediction: if RL were a separate cognitive principle, an RL system should in principle be implementable without trajectory access — a learner that adapts to rewards using only the current state and the reward signal, without any persistent record of past state-action transitions. The framework predicts no such system can produce non-trivial learning: any working RL implementation requires trajectory-class machinery (some form of memory that links rewards back to the states and actions that produced them). Empirically, this matches the actual structure of working RL systems — every successful RL algorithm uses trajectory-class mechanisms, even when this is not its theoretical self-description. The framework's claim is that the trajectory-class machinery is doing the actual learning work; the value-propagation language is description of one operation within the class, not identification of a separate cognitive principle.

Theories that posit RL as a separate cognitive primitive contradict this. Theories that treat RL as one algorithmic family within the trajectory-reuse class — using utility propagation as a particular operation rather than a separate principle — are consistent with the framework. The framework's contribution is to derive *why* utility propagation requires trajectory access: the operation is class-member-internal; cached values are stored in and retrieved from the same logged record that supports inference, not a separate value-storage system.

**Strict goal optimization at the individual agent level is falsified, with a structural source for the failure.** §3.2.10's optimization gap establishes that agents optimize their drifted representation $\widehat{V}^{\text{cont}}$ of the goal function, not the underlying $V^{\text{cont}}$. The falsification has two parts.

The empirical part is largely uncontroversial: descriptive theories of cognition have moved decisively away from strict individual optimization for several decades, with bounded rationality (Simon), heuristics-and-biases (Tversky-Kahneman), satisficing, prospect theory, and ecological rationality (Gigerenzer) all replacing classical optimization in most accounts of how agents actually behave. The framework agrees with this empirical literature.

The novel part — and the actual falsification target — is the structural source of the failure. Most bounded-rationality literature treats the deviation from optimization as an empirical fact about cognitive limits (limited attention, limited memory, limited computation) without deriving why such limits are *structurally inevitable*. The framework derives the optimization gap from the layered self-model (§3.2.10): the agent's representation of $V^{\text{cont}}$ lives in the interpretive layer, which drifts without internal grounding because no internal mechanism provides ground truth against which to correct it. This is a structural derivation, not an empirical observation about cognitive limits.

The falsification targets are positions that retain strict-optimization assumptions despite the bounded-rationality consensus: classical decision theory in normative applications, rational-actor models in economic policy analysis, certain AI alignment formulations that assume the AI's stated objective is what it optimizes, agent designs that treat utility-maximization as a solved problem. The framework predicts these positions will fail in specific ways: misalignment between stated and revealed objectives, drift in goal-pursuit over time without external pressure, specification-gaming patterns in AI systems, and similar. AI alignment researchers have documented many of these patterns under names like "specification gaming," "reward hacking," and "goal misgeneralization"; the framework provides the structural explanation for why they exist and predicts they cannot be fully eliminated by improving optimization, only by external grounding mechanisms that constrain the interpretive layer.

The framework's claim is that strict optimization holds at the population level via selection (agents drifting too far terminate, so surviving populations approximately satisfy the claim) but not strictly for any individual agent at any moment. This is a structural correction to optimization-based normative theories, not a contradiction of bounded-rationality descriptive theories — the latter the framework is fully compatible with.

**Uniform reliability of self-knowledge is falsified.** §3.2.10's layered self-model establishes that factual self-knowledge has internal grounding via logging while interpretive self-knowledge drifts unboundedly without external grounding. The framework predicts a specific pattern: agents are reliable about *what they did* (logged factual self-states accessible by self-supervised learning) and unreliable about *why they did it* (interpretive content with no internal grounding mechanism).

Two uniform positions are falsified, on opposite sides. The classical Cartesian view (introspection is privileged and uniformly reliable) is mostly abandoned in academic philosophy but persists implicitly in research methods that treat first-person self-reports as ground truth across domains — common in some clinical assessment, self-report-based survey research, and certain interpretability approaches in AI that treat model self-explanations as faithful. The opposite position, treating introspection as uniformly unreliable (descended from Nisbett and Wilson's classical confabulation studies), persists in some skeptical traditions in cognitive science and in certain alignment-research framings of chain-of-thought as uniformly post-hoc. The framework rules out both: self-knowledge is neither uniformly reliable nor uniformly unreliable; it is *specifically* reliable for factual content and unreliable for interpretive content.

The distinguishing prediction is the asymmetric pattern itself, which is testable. Compare an agent's accuracy on questions about its own past behavior (factual: "what did you do?") versus its own past reasoning (interpretive: "why did you do it?"). The framework predicts factual accuracy substantially exceeds interpretive accuracy, with the gap widening as elapsed time since the event grows (factual content has a logging-anchored ground truth that decays slowly; interpretive content drifts without anchor). Empirically, this pattern is well-documented in human introspection research — the Nisbett-Wilson tradition's confabulation findings concern interpretive content specifically; behavioral self-reports about discrete past actions are substantially more accurate. Modern introspection research (Wilson, Schwitzgebel, and others) has converged toward something close to the framework's layered structure, though usually without the structural derivation. The framework's contribution is to derive *why* the asymmetry exists: factual content has logging-supported ground truth; interpretive content does not.

Theories of introspection that treat self-knowledge uniformly — whether as uniformly reliable or uniformly unreliable — are falsified by this asymmetry. Theories that already incorporate the asymmetry (Wilson's adaptive unconscious, Schwitzgebel's introspection skepticism applied selectively to interpretive content) are consistent with the framework and have empirical support; the framework adds the structural derivation explaining why the asymmetry is forced rather than contingent.

**Architecturally cost-free introspection is falsified.** §3.2.15 + §3.2.9 establish that metacognition consumes bandwidth like any other inference operation, and there is an irreducible introspective minimum: the agent cannot reduce metacognitive overhead below the level needed to *decide* how to reduce metacognitive overhead — a bandwidth-allocation regress that bottoms out at a non-zero floor.

A defender of consciousness theories may note that the falsification appears to target a strawman: most contemporary theories of consciousness (Global Workspace, Higher-Order Thought, Integrated Information Theory) acknowledge metabolic and computational costs. The framework's target is narrower than "any cost." It targets the architectural-level claim that conscious access or self-awareness operates *separably* from first-order processing — that metacognition can be modeled as a layer that observes first-order operations without competing with them for the same bandwidth. Several formulations within Global Workspace and higher-order traditions treat conscious access this way, even when metabolic costs are acknowledged at a different level. The framework rules this out: metacognition runs on the same internal interface bandwidth as first-order inference, so the two compete directly under any bandwidth-limited regime.

The distinguishing prediction: under cognitive load that consumes internal interface bandwidth, metacognition should degrade in patterns specifically traceable to the bandwidth allocation regress — not generic slowdown, but characteristic failure modes. Specifically, the framework predicts that as first-order load rises, metacognition should fail in a *graceful-degradation* pattern down to the irreducible floor, then exhibit a *catastrophic* loss of self-awareness when the floor is breached (the agent ceases to track its own state because it can no longer afford the metacognitive overhead even to track *that* it isn't tracking). Empirically, this pattern is consistent with documented effects of high cognitive load on metacognitive accuracy in humans (which degrades gracefully under moderate load) and with reports of dissociation under extreme load (which appear suddenly rather than as smooth continuation of degradation). Theories of consciousness that treat metacognitive access as architecturally separable from first-order processing predict graceful degradation throughout, with no characteristic floor; the framework predicts the floor is observable.

The framework predicts a measurable cost in any agent that runs metacognition, traceable to bandwidth competition with first-order inference, and a non-zero floor below which self-awareness cannot drop without losing self-awareness entirely.

**Time-symmetric goal pursuit absent preference is falsified, with a structural source distinguishable from preference.** §3.2.13 plus the trajectory utility structure (§3.2.5) establish that prediction reliability decays with horizon. The agent's utility computations on far-future contributions are weighted less because the predictions themselves are less reliable, not because of impatience or temporal preference.

A reviewer may ask whether this structural source of discounting is observationally equivalent to preference-based discounting — if both predict the same patterns, no test discriminates them. The two are distinguishable, and the framework's claim depends on the distinction. Preference-based discounting (hyperbolic, exponential, or other) is parameter-independent of model quality: an agent discounts at its preferred rate regardless of how well it models the future. Structural discounting is parameter-dependent on model quality: an agent with a better model of the future has less effective discounting because its far-future predictions are more reliable; an agent in a more predictable environment has less effective discounting because there is less unmodeled structure to decay through.

Two distinguishing predictions:

*Prediction A (model-quality variation).* Hold the agent constant; vary the agent's model quality on a domain (through training, expertise development, or domain-specific information access). Preference-based discounting predicts the agent's effective discount rate is unchanged on that domain, since the preference parameter doesn't depend on model quality. Structural discounting predicts the agent discounts less on the domain it models better. Empirically, this pattern is documented: domain experts show reduced temporal discounting in their domains of expertise (financial professionals on financial outcomes, climate scientists on climate-relevant outcomes); the same individuals show standard discounting outside their expertise. These observations are anomalies under pure preference-based discounting and consequences under structural discounting.

*Prediction B (environmental predictability variation).* Hold the agent constant; vary the predictability of the environment. Preference-based discounting predicts unchanged discount rate. Structural discounting predicts less discounting in more predictable environments and more discounting in less predictable ones. Empirically, this matches documented effects: agents in stable environments (low scarcity, stable resources) show reduced temporal discounting compared to the same agents in unstable environments — an effect well-documented in behavioral economics and ecology, often attributed to "scarcity mindset" or "resource availability" but explained mechanistically by structural discounting.

The discount has a structural source independent of preference, and would be present in any agent with no temporal preference at all. Theories that treat temporal discounting as solely a preference parameter contradict the framework's derivation; theories that combine preference with structural sources are consistent with it. The framework's specific prediction is that the structural component is unavoidable — even an agent with zero temporal preference exhibits some discounting because prediction reliability decays.
...
The falsifications are binding because the framework's compression succeeds: §4.1 covers all directly observable properties from §1.1 at the levels of strength named in the methodological note at the start of Chapter 3, with substrate-specific instantiations of those general properties belonging below the framework's level of operation (§4.2). There is no escape route through "the framework is missing something." Either the framework's accounts hold and the literature claims listed above are wrong, or some account is itself wrong — in which case the framework needs revision. The mutual falsifiability is what makes both sides scientific.

### 4.6 Operationalization

The framework's theoretical constructs — cognitive sacrifice, groundedness, outward surprisal, the optimization gap, interpretive drift, frame structure — are framework-level entities, like "free energy" in the Free Energy Principle or "fitness" in evolutionary biology. They do theoretical work even when direct measurement isn't possible. But empirical use of the framework requires *something* — proxies, indicators, partial measurements — that connects the constructs to observable quantities. This section names the proxies that exist for each of the framework's main constructs, what each proxy captures and what it misses, and which falsifications and predictions in §4.4 and §4.5 are testable through current proxies versus which require operationalization that doesn't yet exist.

*Cognitive sacrifice and instantaneous cognitive load* ($\mathcal{K}$, $\kappa$). Defined as the time-integrated mismatch between input information rate and the agent's local regularity ($\kappa(t) = \iota_y(t)/\rho_L^{(y)}(t)$), with $\mathcal{K}$ accumulating across the lifespan. Direct measurement requires both quantities from the agent's perspective, neither externally observable. Proxies: input information rate $\iota_y$ approximates well by Shannon entropy rate of the input stream across the agent's sensory boundary. Local regularity from the agent's perspective is harder, but indicators include prediction error in agent responses, surprise-related neural signatures (P300, mismatch negativity in mammals), behavioral effort indicators (response latency increase, error rate increase under load). Cumulative $\mathcal{K}$ can be proxied by integrated prediction-error-weighted exposure time. The proxies capture qualitative time-course of cognitive demand and support the framework's predictive claim about rising-then-falling property exhibition (§2.2); they miss the from-the-agent's-perspective nature of the formal definition.

*Groundedness* ($\mathcal{G}(M)$). Defined as the relative reduction in KL divergence from the agent's model versus the maximum-entropy prior. Direct measurement requires access to both the true distribution $\mathbb{P}$ and the agent's $q_M$. Proxies: prediction accuracy on held-out sequences from the agent's environment correlates with $\mathcal{G}$; behavioral consistency across similar-but-novel situations also tracks groundedness (well-grounded models predict well in similar-but-novel cases). For artificial agents, $q_M$ may be directly inspectable; for biological agents, behavioral inference is required. The proxy captures relative groundedness across agents or across time within an agent and supports tests of falsification 5 (strict goal optimization) and prediction 5 (differential factual vs. interpretive accuracy); it misses the absolute level of $\mathcal{G}$, since the relative-to-baseline structure is hard to instantiate experimentally.

*Outward surprisal* ($\sigma^{\text{env}}$). Defined as the surprise the environment encounters from the agent's outputs given the environment's current model of the agent. The framework's loop-circulation quantity from §2.1 and the basis for the FEP inversion. Proxies: information-theoretic measures of the agent's output stream from an external observer's standpoint — Shannon entropy of agent outputs given observation history. For laboratory agents, an environmental model can be trained on the agent's past outputs and its prediction error measured on new outputs. For natural agents, behavioral predictability indices, observability metrics from ecology, predator-detection rates for prey species all serve as partial proxies. The proxy supports tests of falsification 1 (FEP inversion) qualitatively — agents with lower outward surprisal should terminate sooner under selection pressure — though the strict form of the falsification requires more sophisticated operationalization than currently exists.

*The optimization gap* ($|\widehat{V}^{\text{cont}} - V^{\text{cont}}|$). Defined as the magnitude of mismatch between the agent's drifted goal-representation and the underlying survival-utility function. Direct measurement requires access to both, neither typically available. Proxies: behavioral patterns indicating goal-misrepresentation. Stated goals diverging from revealed preferences (well-documented in humans), longitudinal drift in goal-pursuit patterns, situations where the agent's behavior produces survival-relevant outcomes inconsistent with self-reported goals. For AI alignment specifically, this construct is what alignment researchers measure under names like "specification gaming," "reward hacking," and "goal misgeneralization" — established empirical work exists. The proxy captures existence and rough magnitude of the gap, supporting tests of falsification 5; it misses the framework's specific claim that the gap is structural rather than implementation-dependent.

*Interpretive drift.* Defined as the change over time in interpretive content (frames, goal-representations, self-interpretation) without external grounding. Proxies: longitudinal measurement of stated beliefs, self-descriptions, value-frames, with comparison of behavior under similar conditions across long timespans. Drift is detectable as systematic change without external pressure. For artificial agents, weight-space drift in interpretive components is directly observable. The proxy captures that drift exists and its rough rate, supporting tests of falsification 9 (frame stability as purely internal); it misses which specific interpretive content is drifting, since interpretive content isn't always cleanly separable from factual content.

*Frame structure.* Defined as the contextual structures stored in $M^{\text{interp}}$ that determine how matched windows are interpreted. Proxies: behavioral evidence of multiple interpretive frames — agents producing different responses to the same input under different priming or context conditions. Frame switching is observable through context-dependent response shifts; frame stability through response consistency across input variations within a frame. The proxy supports tests of prediction 3 (frame agreement as a measurable confidence signal); it misses the structural taxonomy of frames within an agent.

*Architectural and structural constructs.* The framework's structural predictions — figure-8 partition, internal interface bandwidth, the layered self-model layers — are operationalized through anatomical and architectural observation rather than through measurement of theoretical quantities. The internal interface as a measurable distinct feature (prediction 1 in §4.4) is operationalized through neuroanatomy and connectivity studies; the figure-8 architecture's predictions are operationalized through dissection, imaging, and lesion studies. These structural operationalizations are mature for biological agents and tractable for artificial ones. Falsifications and predictions tied to structural claims are therefore testable now; falsifications tied to dynamical quantities like outward surprisal require continued work.

*What remains.* Full operationalization of the framework's theoretical constructs is partial at present: most constructs admit at least one proxy, but no construct admits a measurement protocol that directly captures the formal definition. This is consistent with the framework's status as a theory of cognitive structure operating above substrate-specifics. Full operationalization for any specific agent class is substrate-dependent and requires the substrate-specific extensions flagged in §4.2 and §5.2. Operationalization is therefore partly within the framework (proxies for theoretical quantities) and partly outside (substrate-specific instantiation of those proxies). The framework's contribution is to name the constructs precisely enough that proxies can be designed and improved; the remaining work is the substantial empirical program of building those proxies into validated measurement protocols.
...
**A derivable definition of agency.** Researchers across biology, cognitive science, AI, and organizational theory study agents but use definitions that are stipulated rather than derived. The constructive agent definition (§2.1) gives a sharp criterion: is the entity the finite region enclosed by a minimum-bandwidth, minimum-area cut of a feedback loop? If yes, it is an agent and the framework's derivations apply. The criterion is testable in principle (the relevant topology is observable) and gives a common substrate for cross-disciplinary work.
...
**Evolutionary dynamics.** The framework takes the seed as given at $t_0$, with seed quality $\mathcal{G}(M^{\text{phys}})$ as a starting condition the framework operates on rather than derives. Evolution is the relevant upstream mechanism for biological agents: selection across generations refines $\mathcal{G}(M^{\text{phys}})$ in lineages whose environments make particular seed configurations adaptive. Integrating evolutionary dynamics with the framework would derive how seed quality changes across generations under selection pressure, predict which seed configurations stabilize in which environments, and address questions the framework currently leaves to evolutionary biology — including agent emergence in abiogenetic conditions where pre-agent populations of seed candidates undergo selection until loop-supporting configurations appear. This extension does not affect the compression claim (which operates within-lifespan on agents that exist) but it does fill in where the seed comes from. Substantial work; builds on the framework's seed-quality framing.

**Developmental dynamics.** The framework operates on cognitive operations on existing substrate. For agents whose mature operational substrate substantially exceeds their seed, the developmental phase includes physical construction of the substrate from the seed's instructions — $A_M$ being built, sensory and effector structures growing, the body developing. The framework acknowledges this dimension (§2.5) but doesn't derive its dynamics. Integrating developmental dynamics would derive when developmental windows open and close, what constrains developmental rates, how substrate-construction interacts with cognitive operation during the early developmental phase. Like evolutionary dynamics, this extension does not affect the compression claim but does fill in how operational substrate becomes available. Developmental dynamics are deeply substrate- and lineage-specific (mammalian neural development, plant morphogenesis, artificial-agent training-as-development each have their own dynamics), so this extension would likely take the form of substrate-specific instantiations of common framework-level patterns rather than a single uniform derivation.
...
A behavioral property: the agent eventually ceases producing outputs and dissolves the persistence of its boundary. Termination is preceded by characteristic changes in property exhibition — the falling phase of the predictive claim's predicted shape. The framework will derive that termination is necessary rather than optional: no agent persists indefinitely.
...
The minimum value of information-carrying capacity (bandwidth) along a feedback loop (16); equivalently, the family of surfaces that cut the loop's spatial realization at this minimum value. Formally: among all surfaces that cut the loop's spatial realization into two arcs of the loop graph, the *bottleneck surfaces* are those with minimum bandwidth, denoted $\mathcal{B}(L)$. There may be many such surfaces — bandwidth along a loop is generically constant on stretches, so any cut along the bottleneck stretch achieves the minimum. The constructive agent (18) is defined relative to this family by adding the minimum-area criterion, which selects which bottleneck surface (or surfaces, in the band case) bounds the agent.

### 18. Constructive agent

The finite region enclosed by a minimum-bandwidth, minimum-area cut of a feedback loop (16) in physical space. Two criteria: the surface cuts the loop at minimum bandwidth (selecting the bottleneck (17) value), and among all surfaces doing so, has minimum area (localizing where along the bottleneck stretch the cut sits). The minimum-bandwidth criterion alone does not pick out an agent — many surfaces realize the minimum bandwidth, enclosing finite regions of various sizes. The minimum-area criterion forces the cut to lie at the geometrically narrowest place along the loop, where the boundary is empirically familiar: cell membranes, skin, the surfaces of organisms.

Two cases. *Sharp boundary*: the joint optimization (minimum bandwidth, minimum area) has a unique solution — a single distinguished surface. *Band (gloria) boundary*: the joint optimization has multiple solutions, and the agent's boundary is the union of the equally-minimal surfaces. The band case is non-uniqueness of the optimization, not fuzziness of any single surface. Both cases satisfy the definition; the framework derives different consequences for each.

Formally: given a feedback loop $L$ with spatial realization $|L| \subseteq \Omega$ (where $\Omega$ is the physical space), a constructive agent is a pair $(A, \partial)$ where $A$ is a finite-volume connected component and $\partial$ is a closed connected 2-surface (or, in the band case, a union of such surfaces) separating $|L|$ such that $\partial$ minimizes interface area among bottleneck surfaces:
...
A theoretical proposal due to Karl Friston and others, holding that biological agents act to minimize the surprise they receive from their environment (formally, variational free energy). The framework of this document inverts this proposal: agents act to maintain or increase the surprise they exert on the environment, because outward informativeness is what keeps the loop circulating, and loop circulation is what the agent's existence consists of. Minimization of received surprise is a side effect of environmental modeling, not the optimization target. Under loop-priority (§2.1), the inversion is a derived consequence of the constructive agent definition rather than a separate theoretical posit. References: Friston (2010), Friston et al. (2012).
...
A persistent agent-internal structure that varies with the agent's history, correlates with environmental patterns, and is causally efficacious in producing outputs via the inference mechanism (24). Required for the agent's continued contribution of non-redundant information to the loop (per §2.1 loop-continuation requirement); without it, sustained loop-circulation is statistically improbable. Non-degenerate from agent formation onward, not built from zero — the materials forming the loop carry substrate-encoded model content sufficient for loop closure, on which logging-as-learning extends $M$ across the agent's lifespan (§3.2.2). The world model is distributed across both sub-regions of the agent (per §3.2.6): the durable seed component $M^{\text{phys}}$ lives in $A_{\text{phys}}$ (DNA in biological agents, hardcoded weights in some artificial agents, design-frozen content otherwise) and stays as set at formation; the modifiable component $M^{\text{mod}}_t$ lives in $A_M$ (synaptic and similar substrates in biological agents, trainable weights and activations in artificial agents) and grows via logging. Total $M_t = M^{\text{phys}} \cup M^{\text{mod}}_t$. For agents with degenerate $A_M$ (the viroid case), $M^{\text{mod}}$ is empty and the agent operates entirely on its seed. The framework's claim is functional, not a commitment to representational content in any strong philosophical sense; positions that deny only the label (some readings of enactivism, dynamical-systems cognition, ecological psychology) are compatible with the framework. Subsequent subsections constrain $M$'s form through the inference mechanism class (§3.2.3) and the logging requirement (§3.2.4).

### 24. Inference

The mechanism by which the agent uses its world model (23) to produce outputs given inputs. By §3.2.3, what's forced by the foundation is not a specific algorithm but a class of mechanisms — call it the trajectory-reuse class — characterized by five properties: trajectory dependence (output depends on logged past), context sensitivity (output depends on current input), similarity-based reuse (output uses some similarity criterion to select relevant past content), bounded operation (per-step cost is bounded), and forward projection (output extends matched past content).

Class members include autocorrelation against logged trajectory (the framework's canonical exemplar), kernel methods, learned-embedding proximity search, attention mechanisms over key-value memory, Hopfield-network retrieval, holographic associative memory, and modern retrieval-augmented architectures.

Formally for the autocorrelation exemplar: given history $h_t$ and current window $w := h_{[t-\Delta, t]}$, find $h^*(t) := \arg\max_a \mathrm{corr}(h_{[a, a+\Delta]}, w)$ in the log, then project the immediate continuation $\hat u_t :=$ right-continuous extension at $h^*(t) + \Delta$. The class definition with properties P1–P5 is in Mathematical Deductions §3.2.3.

### 25. Logging

A persistent record of internal state changes during sensing and acting, sufficient to support the agent's inference (24) class member. By §3.2.4, what's forced is a class of mechanisms satisfying persistence, sufficient temporal structure, substrate-supported writability, and inference accessibility. The canonical exemplar is the persistent ordered log $\ell_t$ containing entries for every state change up to time $t$, ordered by time.

Class members include synaptic plasticity, epigenetic marks, calcium dynamics, distributed activation patterns, explicit episodic stores, content-addressable memory traces, log-structured storage, and key-value memory architectures.

By §3.2.4, learning in the framework is just logging — there is no separate learning rule that updates parameters independently of the record; experiences become available to influence future inference simply by being logged in whatever class-member form.
...
The high-information-density sub-region of the agent's region $A$, specialized for processing the informational component of inputs at the cost of being energy-sensitive. Carries the modifiable component $M^{\text{mod}}_t$ of the world model (per §3.2.6's dual-purpose framing) — fast-update content acquired through logging across the agent's lifespan. Joined to the physical sub-region (29) by an internal interface. Forced by §3.2.6 substrate trade-off, except for agents whose entire world model is durable seed content; for these agents (the viroid is the limit case), $A_M$ is degenerate (empty or vanishingly small) and the framework's machinery operating on $A_M$ (working memory, episodic-to-semantic consolidation, etc.) is vacuous. The brain (and analogous fast-update substrates in some lineages) is the canonical case of substantial $A_M$ supporting rich logging-as-learning.

### 29. Physical sub-region ($A_{\text{phys}}$)

The energy-tolerant sub-region of the agent's region $A$, specialized for handling energy-bearing inputs and outputs at the cost of lower information density. Carries the durable seed component $M^{\text{phys}}$ of the world model (per §3.2.6's dual-purpose framing) — slow-update content set at agent formation that persists across the lifespan, with seed quality measured as $\mathcal{G}(M^{\text{phys}})$. DNA in biological agents is the canonical case of durable world-model content stored in $A_{\text{phys}}$. Joined to the mental sub-region (28) by an internal interface. The agent's external interface $\partial_{\text{ext}}$ borders the environment; the internal interface $\partial_{\text{int}}$ separates physical from mental.

### 30. Figure-8 structure

The canonical exemplar of the substrate partition class derived in §3.2.6. A binary partition of the agent's region into mental (28) and physical (29) sub-regions joined by a single internal interface, producing two coupled feedback loops: an external loop between environment and physical sub-region (handling energy-bearing inputs/outputs at low information density) and an internal loop between physical and mental sub-regions (handling high-information-density operations under energy-tolerant conditions). The framework's working reference throughout subsequent chapters.

What's actually forced by §3.2.6 is class membership: any agent under high cognitive amortization in environments with significant energy-bearing inputs must have *some* substrate partition with substrate differentiation, internal interface(s), external shielding, and coupled loops. The figure-8 is the simplest case. Other class members include gradient transitions, distributed mental sub-regions (cephalopod), hierarchical partitions, and multi-tier shielding configurations.

### 31. Episodic memory

In the canonical two-level memory hierarchy, the high-entropy detailed level: detailed records of specific past situations, supporting fine matches at high lookup cost. By §3.2.7 the framework forces a partition with at least two levels of different cost-quality trade-offs; episodic memory is one level in the canonical exemplar. In multi-level or continuous-gradient class members, the analog is the highest-entropy or finest-grained level.

### 32. Semantic memory

In the canonical two-level memory hierarchy, the low-entropy compressed level: summarized patterns over many past situations, supporting fast lookup at coarse match resolution. By §3.2.7 the framework forces a partition with at least two levels; semantic memory is the lower-entropy level in the canonical exemplar. Conversion from episodic (31) to semantic runs as a continuous process in any class member.

### 33. Working memory

A pre-loaded fast-access subset of episodic content (31) for tight-deadline decisions. The canonical exemplar of the fast-access mechanism class derived in §3.2.7: any agent facing both tight deadlines and decisions requiring fine matches must have *some* mechanism for keeping recent or relevant content rapidly accessible. Class members include explicit working memory buffers, sustained activation patterns, attention-based gating, and cache hierarchies. Capacity is bounded by available internal-interface bandwidth divided by per-item maintenance cost regardless of class member.
...
What's derived is the class $\mathcal{I}$ of admissible inference operators, defined by the following properties. Let $h_{[a,b]}$ denote the agent's trajectory record on the interval $[a, b]$, and let $\sigma(h_{[a,b]})$ denote the σ-algebra it generates.

**Class definition.** $\textsc{Infer} \in \mathcal{I}$ if and only if there exists a similarity functional $s: \mathrm{Hist} \times \mathrm{Hist} \to \mathbb{R}_{\geq 0}$ and a forward operator $\textsc{Proj}: \mathrm{Hist} \to S_\partial$ such that:

- (P1, *Trajectory dependence*) $\textsc{Infer}(M, h_t)$ is $\sigma(h_{[t_0, t]})$-measurable, with non-trivial dependence: $\textsc{Infer}$ is not constant on the σ-algebra.
- (P2, *Context sensitivity*) For fixed $M$, $\textsc{Infer}(M, h_t)$ varies with $h_{[t-\Delta, t]}$ for some $\Delta > 0$.
- (P3, *Similarity-based reuse*) The output factors through $s$: there is a measurable function $g$ such that $\textsc{Infer}(M, h_t) = g(\{(s(h_{[a,a+\Delta]}, h_{[t-\Delta,t]}), h_{[a,a+\Delta]}) : a \in [t_0, t-\Delta]\})$.
- (P4, *Bounded operation*) The per-step computational cost is bounded by $\tau_{\textsc{Infer}}(t) \leq \tau_{\max}$ where $\tau_{\max}$ depends on the agent's energy budget via §3.1.5.
- (P5, *Forward projection*) The output is in the image of $\textsc{Proj}$ applied to a history-derived input: $\textsc{Infer}(M, h_t) \in \mathrm{im}(\textsc{Proj})$.

**Canonical exemplar (autocorrelation):** Define a similarity kernel $\mathrm{corr}: \mathrm{Hist} \times \mathrm{Hist} \to \mathbb{R}$. Let $w := h_{[t-\Delta, t]}$ be the recent window of length $\Delta$. Find:
...
This satisfies P1–P5 with $s := \mathrm{corr}$ and $\textsc{Proj}$ the right-continuous extension operator. It is a member of $\mathcal{I}$.

**Other class members include** (informally): kernel methods with non-correlation similarity (e.g., RBF, Mahalanobis), locality-sensitive hashing with similarity-preserving hash families, learned-embedding proximity search, attention mechanisms with softmax-weighted aggregation over key-value memory, Hopfield-network retrieval (where pattern completion implements P3 + P5), holographic associative memory.

**Excluded (non-class):** any operator violating P1 (e.g., constant maps, random output without trajectory access), any operator violating P3 (context-dependent operators that don't use similarity, e.g., last-output replay), any operator whose computational cost is unbounded in $t$ (violating P4), any operator that produces outputs not in the image of trajectory-derived projection (violating P5).

The framework's downstream derivations (§3.2.4 onward) require only $\textsc{Infer} \in \mathcal{I}$, not specific class membership.

#### 3.2.4 Logging as learning

The class $\mathcal{L}$ of admissible logging mechanisms consists of maps $\ell: \mathcal{E}_y \cup \mathcal{E}_u \to S_M$ (where $S_M \subseteq S_A$ is a coordinate projection of the agent's mental state space) satisfying:

- (L1, *Persistence*) For each event $e \in \mathcal{E}_{y,u}$ recorded at time $t_e$, $\ell(e)$ remains accessible (in some substrate-supported sense) for all $t \geq t_e$ during the agent's lifespan, modulo §3.1.4 capacity-driven forgetting.
- (L2, *Sufficient temporal structure*) The σ-algebra of $S_M$ at time $t$ contains $\sigma(\ell(\mathcal{E}_{y,u} \cap [t_0, t]))$ enriched with sufficient temporal information (timestamps, ordering, or local context) to support whichever inference operator $\textsc{Infer} \in \mathcal{I}$ from §3.2.3 the agent uses.
- (L3, *Substrate-supported writability*) The substrate supports the recording rate required by sensing and acting, bounded by §3.1.2 and §3.1.4.
- (L4, *Inference accessibility*) The format and addressability of $\ell$'s output is compatible with the agent's chosen $\textsc{Infer} \in \mathcal{I}$.

**Canonical exemplar:** the injective order-preserving log map producing strict total temporal ordering of events.

**Other class members:** synaptic plasticity with spike-timing-dependent learning (preserves local temporal context), distributed activation patterns with content-addressability (L1 + L4 satisfied via pattern completion), explicit episodic stores, content-addressable memory traces, log-structured storage, key-value memory.

World model amortization runs at rate $\delta_M(\mathcal{G})$:

$$\frac{d\mathcal{G}}{dt}\bigg|_{\text{no update}} = -\gamma\kappa(t)$$

where $\gamma > 0$. Learning is the update rule $M_t \mapsto M_{t+dt}$ producing $d\mathcal{G}/dt > 0$ via prediction-vs-observation comparison applied to entries in $\ell$. The update rule is implemented by whichever class member from $\mathcal{L}$ paired with whichever class member from $\mathcal{I}$ the agent uses.
...
**Class definition.** A partition $\mathcal{P}$ of the agent's region $A$ is admissible if it satisfies:

- (PT1, *Differentiation*) $A$ partitions into a finite collection $\{A_i\}$ of sub-regions such that at least one $A_{\text{phys}}$ has energy-tolerant substrate (low information density per unit volume) and at least one $A_M$ has information-rich substrate (high information density per unit volume).
- (PT2, *Internal interface(s)*) The sub-regions are joined by at least one internal interface $\partial_{\text{int}} \subseteq \partial A_{\text{phys}} \cap \partial A_M$ (or a finite set of such interfaces) with bandwidth $B_{\partial_{\text{int}}}$ tunable independently of the external boundary's bandwidth $B_{\partial_{\text{ext}}}$.
- (PT3, *External shielding*) Inputs from $E$ enter the agent through $\partial_{\text{ext}} \subseteq \partial A_{\text{phys}}$, with the energy component absorbed primarily in $A_{\text{phys}}$. The information-rich sub-region $A_M$ is not directly exposed to high-$\phi$ inputs from $E$.
- (PT4, *Coupled loops*) Any feedback loop with $E$ passes through both $A_{\text{phys}}$ and $A_M$.

**Canonical exemplar (figure-8):** $A = A_{\text{phys}} \sqcup A_M$ with a single internal interface $\partial_{\text{int}}$:
...
Bandwidths satisfy typically $B_{\partial_{\text{int}}} \gg B_{\partial_{\text{ext}}}$.

**Other class members:** sharp binary partitions with discrete $\partial_{\text{int}}$; gradient transitions where $\partial_{\text{int}}$ is replaced by a region of intermediate substrate properties; distributed mental sub-regions with multiple $A_{M,k}$ joined to $A_{\text{phys}}$ via several $\partial_{\text{int},k}$; hierarchical partitions where each $A_{M,k}$ is itself partitioned recursively; multi-tier shielding configurations.
...
Composing §3.2.11 + §3.2.12: clock time correlates with $\mathcal{K}$, $\mathcal{K}$ correlates with property exhibition. The predictive claim of §2.2 (rising phase) is established.
...
For any $\textsc{Infer} \in \mathcal{I}$ from §3.2.3, the operator decomposes:

$$\textsc{Infer} = \textsc{Project} \circ \textsc{Interpret} \circ \textsc{Match}$$

where:
- $\textsc{Match}: \mathrm{Hist} \to \mathrm{Hist}$, finds the matched content (specific form depends on class member: $h^*(t)$ in the autocorrelation exemplar, top-$k$ keys in attention-based members, etc.)
...
Each has its own cost and failure mode. The decomposition follows from class properties P3 (similarity-based reuse — selects what's matched) and P5 (forward projection — extends matched content), which jointly admit interpretation ambiguity for any non-degenerate frame structure. Interpretation requires frames in $\mathcal{F}$ stored in $M^{\text{interp}}$. By §3.2.10, frames drift without external grounding.
...
| 3.2.6 Substrate trade-off, partition class | 3.1.1, 3.1.4, agent definition |
| 3.2.7 Forgetting | 3.1.4, 3.2.4, 3.2.5 |
| 3.2.7 Memory hierarchy | 3.1.4, 3.1.5, 3.2.3 |
| 3.2.7 Working memory | 3.1.4, 3.1.5, 3.2.3, 3.2.7-hierarchy |
| 3.2.8 Habituation | 3.2.4, 3.2.3 |
